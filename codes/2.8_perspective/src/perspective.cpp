...
Matrix getRotMatrix3(char axis, double theta) {
	Matrix m(3, 3);
	if (axis == 'x') {
		m.mat = {
			{1, 0, 0},
...
int main() {
	Vec3 O(0, 0, 0), cameraPos(0, 10, 3); Matrix cameraRot = getRotMatrix3('x', 90*PI/180.0);
	...
	for (int x = -Cw / 2; x < Cw / 2; ++x) {//x, y都是图像(canvas）上面的像素坐标，必须是整数int
		for (int y = -Ch / 2; y < Ch / 2; ++y) {
			Vec3 D = cameraRot * canvas2viewport(x, y);
			Color color = traceRay(cameraPos, D, 1, INF, 3, spheres, lights);
			putPixel(x + Cw / 2, y + Ch / 2, color, image, Cw, Ch);
		}
	}
	createPNG("perspective.png", image, Cw, Ch);
	return 0;
}