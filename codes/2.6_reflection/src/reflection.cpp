...
public:
	...
	Sphere() :center(0., 0., 0.), radius(1.), color(255, 255, 255), specular(-1) {}
};
...
double computeLighting(Vec3 P, Vec3 N, Vec3 V, double s, std::vector<Light> lights) {//P点是O点发出的射线与球体的交点，N是P点的法线单位向量
...
Color traceRay(Vec3 O, Vec3 e, double t_min, double t_max, int recur_depth, std::vector<Sphere>& spheres, std::vector<Light>& lights) {
	...
	if (findCrossSphere) {
		...
		N = N / N.length();//法向量归一化
		Color local_color = closest_sphere.color * computeLighting(P, N, -e, closest_sphere.specular, lights);
...
int main() {
	...
	Sphere sphere3(-2, 0, 4, 1, 0, 255, 0, 10, 1); // 绿色球
	...
	createPNG("reflection.png", image, Cw, Ch);
	return 0;
}